...
(function () {
  ...
  const GTH = (window.GTH = window.GTH || {});
  GTH.sections = GTH.sections || [];   // [{id, group, title, tag, render(el)}]
  GTH.groups = GTH.groups || [];       // orden de grupos en la nav

  /* Registrar una sección de contenido */
  GTH.register = function (section) {
    GTH.sections.push(section);
    if (GTH.groups.indexOf(section.group) === -1) GTH.groups.push(section.group);
  };
